...
Composer send parks files on `queueAttachments` (aligned with `queue`), not a thread-wide bag. `takePendingTurnAttachments` must not merge leftover composer `attachments` into a parked snapshot — a file dropped after send belongs to the next message. Remove / reorder / Send now must move `queueAttachments` with the prompt so a discarded follow-up cannot leak images onto a later turn.
