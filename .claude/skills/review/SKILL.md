...
Orchestration turns isolate vendor MCP so user / claude.ai Linear cannot hang the first find-work turn: Claude `--strict-mcp-config` + `ENABLE_CLAUDEAI_MCP_SERVERS=false`; Codex `-c mcp_servers.<name>.enabled=false` for user servers; OpenCode `enabled: false` in `OPENCODE_CONFIG_CONTENT`; Cursor `settingSources: []` (inline Sideboard MCP still applies). Tickets go through Sideboard `list_issues` / `linear_*`. Do not drop that isolation to “restore” Linear MCP on coordinators. Worktree agents still merge the user’s stdio MCP list. Codex `codex exec` must pass `-c features.apps=false`, `apps._default.enabled=false`, `apps.posthog.enabled=false`, `features.plugins=false`, and `mcp_servers.posthog.enabled=false`. PostHog ships as a Codex plugin that bundles a ChatGPT App; Apps-only isolation still loads that OAuth HTTP MCP and `AuthRequired` kills the turn. Do not re-enable Apps/plugins to “restore” PostHog MCP; use Settings → Connectors + the HTTP API. If a PostHog/MCP AuthRequired still slips through, feed the error back (`looksLikeCodexMcpError`) so the agent continues — do not park the thread on lastError. Do not copy Apps isolation onto Claude / Cursor / OpenCode / Brightsy — they have no ChatGPT Apps surface, and a failed vendor MCP is non-fatal there (Claude thinking / `needsAuth`, Cursor `needsAuth`, OpenCode `needs_auth`). Brightsy has no local MCP injection.
