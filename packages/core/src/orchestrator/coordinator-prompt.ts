...
export const COORDINATOR_TOOL_PLAYBOOK = [
  ...
  '- list_board — Home Kanban (Backlog / Queued / Running / Needs you / Review / Done). Same cards as desktop Home. Linear Backlog defaults to your current cycle (assigned to you this sprint). Filters: query, repoPath, kind, ticketScope (cycle|assigned|all), column, limit. Prefer this for ticket-to-worktree status.',
...
export function coordinatorTurnReminder(opts: {
  ...
  return [
    ...
    '- Status: list_board (Home) or list_threads. Link chats as `[Title](sideboard://thread/<id>)`. Merge only if the user asked.',
  ]
    .filter(Boolean)
    .join('\n');
}
...
export function ensureGlobalCoordinatorCwd(opts?: {
  ...
  const body = [
    ...
    'Typical flow (Home board): list_board → start_board_card (or create_thread sourceType=ticket|pr) → send_to_thread → wait_for_turn (loop while stillRunning) → ask_git create-draft → wait_for_turn. Merge only if the user explicitly asked (`ask_git` merge).',
