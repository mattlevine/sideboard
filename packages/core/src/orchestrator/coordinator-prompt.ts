...
export const COORDINATOR_TOOL_PLAYBOOK = [
  ...
  '- list_board — Home Kanban of worktree chats (In Process / Review / Done). Path to done: no open PR → open PR → archived. Queued/running are activity on the card, not columns. Orchestration chats are not on the board. Filters: query, repoPath, kind, column, limit.',
...
export function coordinatorTurnReminder(opts: {
  ...
  return [
    ...
    '- Status: list_board (worktree Kanban: In Process → Review → Done) or list_threads. Link chats as `[Title](sideboard://thread/<id>)`. Merge only if the user asked.',
  ]
    .filter(Boolean)
    .join('\n');
}
