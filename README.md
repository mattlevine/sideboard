...
https://github.com/mattlevine/sideboard/releases/download/v0.1.149/Sideboard-0.1.149-arm64.dmg
...
- **Linear tickets** — `linear_list_teams`, `linear_search_issues` (`assignee` = `me` / `unassigned` / `all` / user), `linear_get_issue`, `linear_create_issue`, `linear_update_issue`, `linear_comment` (Settings → Issues OAuth; reconnect if you connected before write access). `list_issues` accepts the same `query` / `assignee` filters.
