...
- **Discover** — `list_board` (pulled tickets/PRs/branches + worktrees; `add_to_board` then `start_board_card`; `refresh=true` syncs remotes), `list_workspaces` (path + GitHub slug), `list_branches` / `list_prs` / `list_issues` (Linear or GitHub), `list_threads`
