...
- **Discover** — `list_board` (Home Kanban snapshot, 15m cache; `refresh=true` pulls Linear/GitHub; Linear Backlog defaults to your current cycle), `list_workspaces` (path + GitHub slug), `list_branches` / `list_prs` / `list_issues` (Linear or GitHub), `list_threads`
