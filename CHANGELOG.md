...
- MCP `stop_dev_script` no longer waits out the full timeout when a newer Run request replaces its stop: it follows a later stop that covers the same script (or a stop-all), and fails right away otherwise.
...
- Worktree MCP `run_dev_script` / `stop_dev_script` persist a request for the desktop host instead of spawning in the MCP process, so Run-tab logs and nested Electron match the sidebar Play button.
...
- Cursor Fast mode is optional again (defaults off). Sideboard always sends an explicit `fast=true|false` to the Cursor SDK so Pro+ no longer bills Fast by omission, and the agent picker / composer / Settings expose the toggle for Cursor only.
...
- Release Action retries the Mac zip / `latest-mac.yml` upload with `gh` when GitHub’s `uploads.github.com` returns 500 `Error saving asset` after the dmg already landed.
...
- `linear_update_issue` can set labels (replace names/ids), project, parent, and relations (`blocks` / `blockedBy` / `related` / `duplicate` / `duplicateOf`; `removeRelations` to drop one).
- `github_update_issue` can set labels, project, parent, and blocked-by / blocking relations (`gh issue edit`; relations need `gh` 2.94+).
- `abletime_update_task` can set labels/tags, project, parent, and the wait-on dependency (`set_task_dependency` on the PM MCP endpoint).
...
- Review turns no longer call `ask_user` (Post this review / Keep it in chat) after the recommendation. The review stays in chat so you can read it and type the next step; public GitHub/Linear/AbleTime writes still wait until you ask.

## [0.1.183] - 2026-09-11

### Fixed

- Streaming log artifacts no longer steal the right-column tab. A new tab still focuses on first open; later appends refresh the log in place if you switched away.
...
- Review confirmation now covers tickets as well as PRs. Findings stay in chat until **Post this review / Keep it in chat**, including the implementation tab while a Review sibling is open, so you can work through feedback before any GitHub/Linear/AbleTime comment or update.
...
- Review worktrees keep the **review** badge only — no yellow left stripe on the sidebar or Home Kanban.
...
- Fresh-session seeds are bounded: summaries always survive, only the last few turns keep full tool input/output, older turns keep one-line tool labels, thinking is never replayed, and the body is capped (~60k chars) instead of re-sending the whole compacted transcript.

### Fixed

- PR status shows **Changes requested** (not Rejected) for `CHANGES_REQUESTED`, and **Needs approval** until a review actually approves.

### Removed

- Unused `loadAgentInstructions` / `withAgentInstructions` helpers (agent CLIs already load `CLAUDE.md` / `AGENTS.md` from the worktree).
...
- Slack Listen claim fallback no longer races a late `claim_denied` into a second inbound turn on another Mac.
