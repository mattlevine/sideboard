...
- Release: follow [`.claude/skills/release/SKILL.md`](../../.claude/skills/release/SKILL.md). “Create a new release” = Electron GitHub Release + build CLI/core (human publishes npm) + README/`site/` + commit/push + Fly deploy if `site/` or the relay changed. After the Mac pack, retarget `vX.Y.Z` onto the Release commit — electron-builder tags `origin/main`, not the worktree. Any long worktree command: `/long-running` (Sideboard product skill; this repo also has [`.claude/skills/long-running/SKILL.md`](../../.claude/skills/long-running/SKILL.md)) — `node scripts/detached-job.cjs start <id> -- …` then loop `wait` (45s slices). The desktop opens a `type=log` pane from wait/stop JSON. `stop_job` if it hangs. Connector CLIs: `.context/cli/` + slice, never raw `--json` / `--expand` in the tool result. npm publish uses `--no-git-checks`; desktop GitHub Releases are signed/notarized from `apps/desktop` (**Apple Silicon / arm64 only**). The Release Action retries the Mac zip / `latest-mac.yml` with `gh release upload` when GitHub 500s `Error saving asset` after the dmg uploaded. Marketing site + Slack relay: [deploy.md](deploy.md).
