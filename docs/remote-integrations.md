...
Connections are owned by Sideboard, not per-agent MCP. Optional connector tokens inject env into worktree agents; prefer official CLIs (`vercel`, `supabase`, `sentry-cli`) or the PostHog HTTP API. Write CLI/HTTP dumps to `.context/attachments/` (same local scratch as file attachments) and read a slice. Do not add vendor MCPs.
