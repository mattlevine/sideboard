...
export const REVIEW_REQUEST_PREFILL =
...
The comments will be presented as a review report in chat (and can be turned into Sideboard diff comments). You should avoid providing unnecessary location details in the comment body. Always keep the line range as short as possible for interpreting the issue. Avoid ranges longer than 5–10 lines; instead, choose the most suitable subrange that pinpoints the problem.
...
Write a clear list of issues found, along with the file path (and line range when known). **IMPORTANT: Only report ONE finding per unique issue.**
...
/**
 * Ensure the editable review request file exists; return its current contents.
 * Preserves user edits when the file is already present.
 */
